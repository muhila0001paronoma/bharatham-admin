...
import { Search, Send, Check, CheckCheck, Info, Plus, X, Users, User as UserIcon, UserPlus, UserMinus, Trash2, Shield } from 'lucide-react';
...
export default function Chats() {
  ...
  const [newChatType, setNewChatType] = useState(null); // 'personal' or 'group'
  ...
  const [messageReactions, setMessageReactions] = useState({}); // messageId -> list of reactions
  const [reactionCounts, setReactionCounts] = useState({}); // messageId -> reaction summary
    ...
    try {
      if (myReaction) {
        if (myReaction.emoji === emoji) {
          ...
          // Change emoji: delete old, add new
          await messageService.deleteMessageReaction(myReaction.id);
      ...
      if (response.success && response.data) {
        const sortedMessages = response.data
          ...
          .map(m => {
            ...
            // Always fetch reactions for all messages
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (newChatType === 'personal') {
          ...
          // Add all selected users for group
